...
//change project area to aharic when lang is in amhracic

for (
  let i = 0;
  i < document.getElementsByClassName("ulcontainer").length;
  i++
) {
  ...
  ulContainer.addEventListener("click", () => {
    window.location.href =
      window.location.href.split("#")[0] + `/${image_div.dataset.id}`;
  });

  let imageSrc = image_div.style.backgroundImage
    .replace(/url\((['"])?(.*?)\1\)/gi, "$2")
    .split(",")[0];
  let image = new Image();
  image.src = imageSrc;
...
{
  document.querySelectorAll('.projectarea').forEach(item => {
    item.innerHTML = 'የፕሮጀክት ስፋት: ';
  })
}
else
{
  document.querySelectorAll('.projectarea').forEach(item => {
    item.innerHTML = 'Project area: ';
  })
}